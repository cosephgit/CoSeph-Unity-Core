...
namespace CoSeph.Core
{
    ...
    public class CSParallax : MonoBehaviour
    {
        ...
        public void AddLayer(CSParallaxLayer layerAdd)
        {
            if (layerAdd != null)
            {
                if (layerAdd.layer != null && layerAdd.distance > 0)
                {
                    ...
                    else
                    {
                        ...
                        layerAdd.scale = 1f / layerAdd.distance;
        ...
        private void InitFollow()
        {
            ...
            // - Unanchored: parallax is relative to world origin
            if (_followAnchor)
                ...
                if ((_follow.position - _followPos).sqrMagnitude > 0.0001f)
                {
                    ...
                    for (int i = 0; i < _layersActive.Count; i++)
                    {
                        if (_layersActive[i].layer != null) // in case it's deleted
                        {
                            ...
                            _layersActive[i].layer.position = _layersActive[i].followOffsetCurrent + parallaxDelta;
                        }
                    }
                }
            }
